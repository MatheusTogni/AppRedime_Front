<template>
    ...
    <v-row justify="center" align="center" class="min-vh-100">
      <v-col cols="12" class="text-center">
        <div class="header-section mb-8">
          <v-icon size="80" color="#af1d36" class="calendar-icon mt-4"
            >mdi-calendar</v-icon
          >
          <h1 class="app-title mb-2">Eventos da Igreja</h1>
          <p class="app-subtitle mb-6">
            Confira os próximos eventos e atividades da nossa comunidade
          </p>
        </div>
              ...
              <v-card elevation="4">
                <v-card-text class="pa-0">
                  ...
                  <div class="calendar-header-grid">
                    <div v-for="dia in diasSemana" :key="dia" class="calendar-header-day">
              ...
              <v-card class="empty-state-card" elevation="4">
                <v-card-text class="pa-8">
                  ...
                  <p class="text-body-1 text-grey-lighten-2">
                    Não há eventos programados no momento. Volte em breve para conferir as
                    novidades!
                  </p>
                </v-card-text>
              </v-card>
  ...
  <v-dialog v-model="modalEvento" max-width="500px">
    <v-card>
      <v-card-title
        class="text-h5"
        :style="{ backgroundColor: eventoSelecionado?.cor || '#af1d36', color: 'white' }"
      >
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="grey" variant="flat" @click="modalEvento = false"> Fechar </v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
          ...
          <v-list class="bg-transparent">
            ...
            >
              ...
              <div>
                ...
                <v-chip :color="evento.cor" size="small" variant="outlined" class="mt-2">
  ...
  computed: {
    ...
    diasCalendario() {
      ...
      for (let i = primeiroDiaSemana - 1; i >= 0; i--) {
        ...
        const dataString = formatLocalDate(
          new Date(this.anoAtual, this.mesAtual - 1, dia)
        );
        dias.push({
          numero: dia,
          data: dataString,
        ...
        if (response && response.data) {
          this.eventos = (response.data.eventos || []).map((ev) => {
            ...
            if (copy.data_evento) {
              ...
              copy._localDate = `${year}-${month.padStart(2, "0")}-${day.padStart(
                2,
                "0"
              )}`;
